%%%-----------------------------------------------------------------------
%
%   Description: The parameter estimation used EKF and this work refers to
%                Garri's work. This implementation is for all parameters 
...
p = [3 3 0.05 0.015 0.03 0.0254];   % The true value of parameter
...
init = [0 0 0 3 3 0.05 0.015 0.03 0.0254];    % true values for Dt St Uf...
...
sol     = ode45(@hs_odeEKF_all_Para,[0 200],init,options);                                                                
...
%   The initialization for parameters
xe_init(state_dim+1:para_dim+state_dim,:) = [4 0 1 1 1 1]; 
...
Pe_init = diag([25 25 25 1e-1 1e-1 1e-1 1e-1 1e-1 1e-1]);
...
for t = 2:size(ytrue,2)
    ...
    Fk = Jacobian_HeatShock_allparams(xe_ekf(:,t-1));
    
    % **2** Advance time
    solEKF = ode45(@hs_odeEKF_all_Para,[0 deltaT],xe_ekf(:,t-1),options);
...
%%  Plot Graphs
figure,
plot(3*ones(1,size(ytrue,2)),'b--','LineWidth',2);hold on;
...
figure,
plot(3*ones(1,size(ytrue,2)),'b--','LineWidth',2);hold on;
plot(xe_ekf(5,:),'r--','LineWidth',2);hold on;
xlabel('Time','Fontsize',20);
...
plot(xe_ekf(6,:),'r--','LineWidth',2);hold on;
xlabel('Time','Fontsize',20);
...
figure,
plot(0.015*ones(1,size(ytrue,2)),'b--','LineWidth',2);hold on;
plot(xe_ekf(7,:),'r--','LineWidth',2);hold on;
xlabel('Time','Fontsize',20);
...
figure,
plot(0.03*ones(1,size(ytrue,2)),'b--','LineWidth',2);hold on;
plot(xe_ekf(8,:),'r--','LineWidth',2);hold on;
xlabel('Time','Fontsize',20);
...
figure,
plot(0.0254*ones(1,size(ytrue,2)),'b--','LineWidth',2);hold on;
plot(xe_ekf(9,:),'r--','LineWidth',2);hold on;
xlabel('Time','Fontsize',20);
