...
% bandwidth of regularized filter
h = (8*c^(-1)*(d + 4)*(2*sqrt(pi))^d)^(1/(d + 4))*N^(-1/(d + 4));
...
for k = 1 : tf
    ...
    %% Particle filter(SIR)
    for i = 1 : N
        xpartminus(i) = 0.5*xpart(i)+...
            25*xpart(i)/(1 + xpart(i)^2)+8*cos(1.2*(k-1))+sqrt(Q)*randn;
    ...
    for i = 1 : N
        xpartminusReg(i) = 0.5*xpartReg(i)+...
            25*xpartReg(i)/(1+xpartReg(i)^2)+8*cos(1.2*(k-1))+sqrt(Q)*randn;
    ...
    for j = 1 : NReg
        ...
        for i = 1 : N
            ...
            if normx < h
                qreg(j) = qreg(j)+...
                    q(i) * (d + 2) * (1 - normx^2 / h^2) / 2 / c / h^d / det(A);
            end
        end
    end
