...
N = 300;        % number of used particles
...
tspan = linspace(0,200,400);
deltaT = tspan(2) - tspan(1);
trueState = deval(sol,tspan);

% Adding noise for state
trueState(1,:)=trueState(1,:) + sqrt(covState1)*randn(1,size(trueState,2));
...
% partcles
paraSmc = zeros(6,N);
% paraSmc(1,:) = 1 + sqrt(1)*randn(1,N);      % Initialization for Ku
...
paraSmc(1,:) = 1 + sqrt(100)*randn(1,N);                % Initialization for Ku
paraSmc(2,:) = 4 + sqrt(100)*randn(1,N);     % Initialization for As
paraSmc(3,:) = 3 + sqrt(100)*randn(1,N);              % Initialization for Ks
paraSmc(4,:) = 2 + sqrt(100)*randn(1,N);            % Initialization for A0
paraSmc(5,:) = 0 + sqrt(100)*randn(1,N);      % Initialization for Kd
paraSmc(6,:) = 2 + sqrt(100)*randn(1,N); 
